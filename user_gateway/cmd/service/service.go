...
import (
	...
	http "parallelSystems/user_gateway/pkg/http"
...
var fs = flag.NewFlagSet("user_gateway", flag.ExitOnError)
...
func Run() {
	...
	/*	if *zipkinURL != "" {
		...
		collector, err := zipkingoopentracing.Wrap().
		if err != nil {
			logger.Log("err", err)
			os.Exit(1)
		}
		defer collector.Close()
		recorder := zipkingoopentracing.NewRecorder(collector, false, "localhost:80", "user_gateway")
